...
begin
    ...
    if not exists (
        ...
        where f.ID = @Record_ID
            and f.FlagLoaded = cast(1 as bit) -- cast не нужно, если тип уже bit
    )
    begin
        set @ErrorMessage = 'Ошибка при загрузке файла, проверьте корректность данных'
        raiserror(@ErrorMessage, 3, 1) -- использовать конструкцию try/catch для обработки ошибок
        return
    end
    ...
    from syn.SA_CustomerSeasonal cs
        join dbo.customer as c on c.UID_DS = cs.UID_DS_Customer -- Название таблицы писать единообразно
            and c.ID_mapping_DataSource = 1
        join dbo.Season as s on s.Name = cs.Season
        join dbo.Customer as c_dist on c_dist.UID_DS = cs.UID_DS_CustomerDistributor
            and cd.ID_mapping_DataSource = 1 -- cd ?
        join syn.CustomerSystemType as cst on cs.CustomerSystemType = cst.Name
    where try_cast(cs.DateBegin as date) is not null
        and try_cast(cs.DateEnd as date) is not null
        and try_cast(isnull(cs.FlagActive, 0) as bit) is not null -- Повторяющийся try_cast лучше вынести в CTE или представление
    ...
    when matched
        and t.ID_CustomerSystemType <> s.ID_CustomerSystemType then -- t ?
        update
        set
    ...
    begin
        ...
        select top 100
            ...
            ,isnull(format(try_cast(DateBegin as date), 'dd.MM.yyyy', 'ru-RU'), DateBegin) as 'Дата начала' --  try_cast внутри format может быть нестабилен — лучше приводить заранее
            ,isnull(format(try_cast(DateEnd as date), 'dd.MM.yyyy', 'ru-RU'), DateEnd) as 'Дата окончания' -- Аналогично предыдущему
